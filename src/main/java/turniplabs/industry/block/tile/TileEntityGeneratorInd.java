...
public class TileEntityGeneratorInd extends TileEntityEnergyConductor implements IInventory {
    private ItemStack[] contents;
    public int maxBurnTime = 0;
    public int currentBurnTime = 0;
    public ItemStack currentFuel;

    public TileEntityGeneratorInd() {
        setCapacity(4096);
        setTransfer(32);
    ...
    public void updateEntity() {
        ...
        if (this.currentBurnTime == 0) {
            this.maxBurnTime = this.currentBurnTime = this.getBurnTimeFromItem(this.contents[1]) / 5;
            if (this.currentBurnTime > 0) {
                currentFuel = this.contents[1];
                onInventoryChanged();
